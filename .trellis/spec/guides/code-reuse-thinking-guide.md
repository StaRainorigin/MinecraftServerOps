...
**Prevention**: When adding a new value to a Python enum (e.g., `LoaderType`), search for ALL if/elif/else chains that switch on that type and add explicit branches. Don't rely on `else` being correct for new values.

**MC-SRE example**: When adding `LoaderType.PAPER` or `LoaderType.NEOFORGE`, you must update:
1. `builder/fetcher/sources.py` — URL 构造
2. `builder/install/installer.py` — 安装器执行
3. `builder/launch/script_gen.py` — 启动脚本生成

---

## MC-SRE 重复代码陷阱

### 陷阱 1：多引擎 `_human_size()` 重复

`_human_size()` 工具函数在 `downloader.py`、`downloader_httpx.py`、`publisher.py` 中各有一份拷贝。新增需要字节格式化的地方应考虑提取到共享位置。

### 陷阱 2：`_parse_major_version()` 重复

Java 版本号解析函数在 `installer.py` 和 `java_probe.py` 中各定义了一份。修改一处时容易忘记另一处。

### 陷阱 3：下载引擎签名必须一致

aria2c 和 httpx 两种引擎的 `fetch_all()` 签名必须保持一致。修改其中一个的参数时，必须同步另一个。参考 `builder/fetcher/__init__.py` 的 `get_fetcher()` 路由逻辑。

---

## Asymmetric Mechanisms Producing Same Output

**Problem**: When two different mechanisms must produce the same output (e.g., aria2c 和 httpx 两种引擎下载同一个 mod), structural changes (URL 构造、文件名解码逻辑) 只在一个引擎中修复时，另一个引擎仍使用旧逻辑。

**Prevention**:
- 共享 URL 构造层：`sources.py` 是两种引擎共享的单一来源
- 共享文件名解码逻辑：`cf_resolver.py` 的 `_decode_filename()` 是唯一解码入口
- 当修复一个引擎的 bug 时，搜索另一个引擎是否有相同问题
